...
from raft_evaluation import raft_evaluate
...
if __name__ == '__main__':
    ...
    parser.add_argument('--peft_checkpoint_path',type=str,default='')
    ...
    if 'peft_checkpoint_path' not in hparam:
        hparam.peft_checkpoint_path =''
    ...
    if arg_.peft_checkpoint_path == '':
        print("no PEFT checkpoint loaded!!!")
    if 'peft_method' not in hparam:
        hparam.peft_method = None
    ...
    args_dict = dict(
        ...
        peft_checkpoint_path = arg_.peft_checkpoint_path,
        opt_level='O1',
    ...
    if args.checkpoint_path!="":
        ...
        if hparam.peft_method == None:
            ...
            for key, value in t5_3b.items():
                t0_new['model.'+key] = value
            model.model.base_model.load_state_dict(t0_new, strict=False)
    else:
        model = T5_small(args)
    ...
    if (args.mode == 'evaluate') or (args.mode == 'rationale_evaluate'):
        import time
        start = time.time()
        if 'raft' in args.wandb_run_name:
            raft_evaluate(args, model,tokenizer)
        else:
        ...
        train_params = dict(
            ...
            logger=wandb_logger,
            callbacks = callbacks,
            strategy=pl.strategies.DeepSpeedStrategy(config="./deepspeed.json"),
            log_every_n_steps=1,
        )
        ...
        if hparam.peft_method == None:
            param_dict = {}
            for name, param in tqdm(model.model.named_parameters()):
                param_dict[name]=param.clone().detach().cpu()
            torch.save(param_dict, hparam.output_dir[:-3]+'_'+hparam.wandb_run_name+'.pt') 
        else:
            model.model.save_pretrained(hparam.output_dir[:-3]+'_'+hparam.wandb_run_name)